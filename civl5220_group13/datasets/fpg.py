...
class FloorplanGraphDataset(Dataset):
    ...
    def flip_and_rotate(self, v, flip, rot, shape=256.0):
        v = self.rotate(np.array((shape, shape)), v, rot)
        if flip:
            x, y = v
            v = (
                (shape / 2 - abs(shape / 2 - x), y)
                if x > shape / 2
                else (shape / 2 + abs(shape / 2 - x), y)
            )
        return v

    # rotate coords
    def rotate(self, image_shape, xy, angle):
        org_center = (image_shape - 1) / 2.0
        rot_center = (image_shape - 1) / 2.0
        org = xy - org_center
        a = np.deg2rad(angle)
        new = np.array(
            [
    ...
    def build_graph_door_as_dents(self, rms_type, fp_eds, eds_to_rms, out_size=128):
        ...
        for d in doors_inds:
            ...
            for r in range(len(nodes)):
                if r not in doors_inds:
                    is_adjacent = any(
                        [True for e_map in eds_to_rms if (r in e_map) and (d in e_map)]
                    )
                    if is_adjacent:
            ...
            for l in range(len(nodes)):
                if l > k:
                    is_adjacent = any(
                        [
                            True
                            for d_key in door_to_rooms
                            if (k in door_to_rooms[d_key])
                            and (l in door_to_rooms[d_key])
                        ]
                    )
                    if is_adjacent:
                        if "train" in self.split:
        ...
        for k in range(len(nodes)):
            ...
            doors_im = doors_im.filter(ImageFilter.MinFilter(3)).resize(
                (out_size, out_size)
            )
